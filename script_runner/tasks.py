...
from script_runner import eval_env, constants
...
def create_process_config(process, operation_kwargs):
    ...
    for key, val in env_vars.items():
        ...
        if isinstance(val, (dict, list, set, bool)):
            ...
            output_env_vars[key] = envvar_value
        else:
            output_env_vars[key] = str(val)
    ...
    if schema in ['http', 'https']:
        ...
        if response.status_code == 404:
            raise NonRecoverableError(
                'Failed downloading script: {0} (status code: {1})'
